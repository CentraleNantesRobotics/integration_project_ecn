...
PidObject::PidObject() : error_(3, 0), filtered_error_(3, 0), error_deriv_(3, 0), filtered_error_deriv_(3, 0)
{
  ...
  node_priv.param<std::string>("topic_from_dplant", topic_from_dplant_, "dstate"); //Joe
  node_priv.param<std::string>("setpoint_topic", setpoint_topic_, "setpoint");
  node_priv.param<std::string>("dsetpoint_topic", dsetpoint_topic_, "dsetpoint"); //Joe
  ...
  node_priv.param<bool>("d_input_enable", d_input_enable_, false); //Joe
  ...
  ros::Subscriber dplant_sub_ = node.subscribe(topic_from_dplant_, 1, &PidObject::plantStateDerivateCallback, this); //Joe
  ros::Subscriber setpoint_sub_ = node.subscribe(setpoint_topic_, 1, &PidObject::setpointCallback, this);
  ros::Subscriber dsetpoint_sub_ = node.subscribe(dsetpoint_topic_, 1, &PidObject::dsetpointCallback, this); //Joe
...
//Joe
void PidObject::dsetpointCallback(const std_msgs::Float64& dsetpoint_msg)
{
  dsetpoint_ = dsetpoint_msg.data;
  last_setpoint_msg_time_ = ros::Time::now();
  new_state_or_setpt_ = true;
}
...
//Joe
void PidObject::plantStateDerivateCallback(const std_msgs::Float64& dstate_msg)
{
  plant_dstate_ = dstate_msg.data;
...
void PidObject::doCalcs()
{
  ...
  if (new_state_or_setpt_)
  {
    ...
    if(!d_input_enable_) //Joe
        error_deriv_.at(0) = (error_.at(0) - error_.at(1)) / delta_t_.toSec();
    else{ //Joe
#ifdef DINPUTDEBUG
        ROS_WARN("ça y est");
#endif
        error_deriv_.at(0) = dsetpoint_ - plant_dstate_; //Joe
    }
